...
set ENTRIES=src/openai-server/start.ts
npx -y nodemon --watch "src/**/*.{ts,js,mjs,cjs}" --watch "rollup.*.{js,mjs}" --ext "js,ts,cjs,mjs" --ignore "tmp,node_modules" --exec "npm run build && node dist/openai-server/start.cjs"
